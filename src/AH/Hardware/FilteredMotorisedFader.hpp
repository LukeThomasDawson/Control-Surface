...
#include <LTD/MotorisedFaderInterfaceMaster.hpp>
...
class GenericFilteredMotorisedFader {
  public:
    /**
     * @brief   Construct a new GenericFilteredMotorisedFader object.
     *
     * @param   analogPin
     ...
     */
    GenericFilteredMotorisedFader(uint8_t faderIndex, MappingFunction mapFn,
                          AnalogType initial = 0)
        : faderIndex(faderIndex), mapFn(std::forward<MappingFunction>(mapFn)),
    ...
    bool getTouched() const {
        return MotorisedFaderInterfaceMaster.getFaderTouched(faderIndex);
    }
...
//        AnalogType value = ExtIO::analogRead(faderIndex);

        AnalogType value = MotorisedFaderInterfaceMaster.getFaderValue(faderIndex);
...
template <uint8_t Precision = 10,
          ...
          uint8_t IncRes = MaximumFilteredAnalogIncRes<
              FilterShiftFactor, FilterType, AnalogType>::value>
class FilteredMotorisedFader
    : public GenericFilteredMotorisedFader<AnalogType (*)(AnalogType), Precision,
                                   FilterShiftFactor, FilterType, AnalogType,
                                   IncRes> {
  public:
    /**
     * @brief   Construct a new FilteredMotorisedFader object.
     *
     * @param   faderIndex
     *          The analog pin to read from.
     * @param   initial 
     *          The initial value of the filter.
     */
    FilteredMotorisedFader(uint8_t faderIndex, AnalogType initial = 0)
        : GenericFilteredMotorisedFader<AnalogType (*)(AnalogType), Precision,
                                FilterShiftFactor, FilterType, AnalogType,
                                IncRes>(faderIndex, nullptr, initial) {}

    /**
     * @brief   Construct a new FilteredMotorisedFader object.
     * 
     * **This constructor should not be used.**  
     * It is just a way to easily create arrays of FilteredMotorisedFader objects, and
     * initializing them later. Trying to update a default-constructed or 
     * uninitialized FilteredMotorisedFader object will result in a fatal runtime error.
     */
    FilteredMotorisedFader() : FilteredMotorisedFader(NO_PIN) {}
     ...
     */
    void invert() {
        constexpr AnalogType maxval = FilteredMotorisedFader::getMaxRawValue();
